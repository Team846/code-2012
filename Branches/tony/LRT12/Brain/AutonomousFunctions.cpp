...
bool AutonomousFunctions::doFunkyMoves()
{	
	m_action->drivetrain->rate.drive_control = true;
	m_action->drivetrain->rate.turn_control = true;
	
	m_action->drivetrain->rate.desiredDriveRate = 0.0;
	m_action->drivetrain->rate.desiredTurnRate = 0.5;
	
	static int turnedAmount;
	static int counter;
...
const AutonomousFunctions::autonomousStage
		AutonomousFunctions::SHOOT_THEN_BRIDGE_THEN_DANCE[SHOOT_THEN_BRIDGE_THEN_DANCE_LENGTH] =
		{ INIT, ADJUSTABLE_DELAY, /*KEY_TRACK, AIM,*/SHOOT, DELAY_HALF_SEC,
				DROP_WEDGE, MOVE_BACK_INIT, WAIT_FOR_POSITION, DELAY_HALF_SEC,
				RAISE_WEDGE, DANCE, DONE };
...
void AutonomousFunctions::loadQueue()
{
	...
	for (uint8_t i = 0;; ++i)
	{
		...
		autonomousStage s = SHOOT_THEN_BRIDGE_THEN_DANCE[i];
