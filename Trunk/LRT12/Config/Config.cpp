...
Config::Config() :
	configLastReadTime_(0), ds(*DriverStation::GetInstance()),
			buildNumKey("Number"), runNumKey("RunNumber"),
...
void Config::Load()
{
	...
	if (!hasRun)
	{
		if (Get<int> (configSection, buildNumKey, -1) == Build::GetNumber())
			Set<int> (configSection, runNumKey,
					Get<int> (configSection, runNumKey, 0) + 1);
		else
		{
...
void Config::UpdateAssignableDials()
{
	for (int i = 0; i < kNumAnalogAssignable; ++i)
	{
		if (analogAssignmentKeys[i].length() != 0)
		{
			...
			Console::GetInstance().PrintMultipleTimesPerSecond(0.25,
					"Assgn:%s %s= %f\n", analogAssignmentSections[i].c_str(),
					analogAssignmentKeys[i].c_str(), newValue);
			pf.Finish();
...
{
	return newConfigData->find(section) != newConfigData->end()
			&& (*newConfigData)[section].find(key)
...
template double Config::Get<double>(string section, string key,
		double defaultValue);
template string Config::Get<string>(string section, string key,
		string defaultValue);
...
template<typename T> T Config::Get(string section, string key, T defaultValue)
{
	...
	else
	{
		...
		printf("Using default value for %s %s which is %s\n", section.c_str(),
				key.c_str(), Util::ToString<T>(defaultValue).c_str());
