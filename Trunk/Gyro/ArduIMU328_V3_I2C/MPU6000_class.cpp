...
// Read AccelX
    byte_H = _SPI_read(MPUREG_ACCEL_XOUT_H);
    byte_L = _SPI_read(MPUREG_ACCEL_XOUT_L);
    accelX = (byte_H<<8)| byte_L;
    // Read AccelY
    byte_H = _SPI_read(MPUREG_ACCEL_YOUT_H);
    byte_L = _SPI_read(MPUREG_ACCEL_YOUT_L);
    accelY = (byte_H<<8)| byte_L;
    // Read AccelZ
    byte_H = _SPI_read(MPUREG_ACCEL_ZOUT_H);
    byte_L = _SPI_read(MPUREG_ACCEL_ZOUT_L);
    accelZ = (byte_H<<8)| byte_L;
    
    // Read Temp
    byte_H = _SPI_read(MPUREG_TEMP_OUT_H);
    byte_L = _SPI_read(MPUREG_TEMP_OUT_L);
    temp_C = ((int)byte_H<<8)| byte_L; 
     
    // Read GyroX
    byte_H = _SPI_read(MPUREG_GYRO_XOUT_H);
    byte_L = _SPI_read(MPUREG_GYRO_XOUT_L);
    gyroX = (byte_H<<8)| byte_L;
    // Read GyroY
    byte_H = _SPI_read(MPUREG_GYRO_YOUT_H);
    byte_L = _SPI_read(MPUREG_GYRO_YOUT_L);
    gyroY = (byte_H<<8)| byte_L;
    // Read GyroZ
    byte_H = _SPI_read(MPUREG_GYRO_ZOUT_H);
    byte_L = _SPI_read(MPUREG_GYRO_ZOUT_L);
...
void MPU6000_Class::calculate()
{
  ...
  // Euler angles calculation from rotation matrix - DEW 1/19/2012
  /*
  MPU_roll = atan2(DCM[2][1],DCM[2][2]);
  MPU_pitch = asin(-DCM[2][0]);
  */
  
  MPU_pitch = -atan2(DCM[2][1],DCM[2][2]);
  MPU_roll = asin(-DCM[2][0]);
